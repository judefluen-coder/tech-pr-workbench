...
Local-first topic video intelligence, Chinese subtitle translation, and authorized clip preparation for tech PR teams.

Tech PR Workbench 是一个本地运行的主题视频情报与剪辑工作台，面向科技公司 PR、内容运营和创始人办公室。它把“AI 采访”升级为可切换的主题模板：你可以追踪 AI 采访、科技高管访谈、竞品发布/演示，也可以复制模板改成自己的行业监测主题。系统会保留原始链接，识别人物或对象，生成中文摘要；当你选择某条视频后，可以下载授权素材、生成中文字幕、挑选高光片段，并导出带字幕的剪辑序列视频。

> 默认定位：个人或小团队本地工具。请只下载、剪辑和再发布你有权处理的素材。

## Highlights

- 选择主题模板：内置 AI 采访、科技高管访谈、竞品发布/演示。
- 按日期区间抓取主题候选：YouTube 优先，B 站补充。
...
这个页面用于每天判断“有哪些视频值得进入下一步”。默认展示 AI 采访模板，也可以切换到科技高管访谈或竞品发布/演示：

- 主题模板：选择 AI 采访、科技高管访谈、竞品发布/演示；复制内置模板后可编辑关键词。
- 顶部日期区间：选择要看的日期，例如昨天，或 6 月 1 日到 6 月 13 日。
- 抓取按钮：按当前主题运行 YouTube/B 站发现流程，刷新候选视频。
- 数据概览：显示候选数量、可处理数量、处理中数量、已可剪辑数量。
- 来源状态：告诉你 YouTube、B 站、RSS 等来源当前是否可用。
- 视频列表：逐条展示标题、来源、人物/对象识别、摘要、观看量和时长。
- 操作按钮：打开原始链接，或点击“下载并翻译”进入处理流程。

追踪标签有三类：

- `追踪：某人/对象`：命中了当前模板的重点人物、公司或产品池。
- `识别：某人`：系统从标题、简介或字幕中识别出的采访嘉宾。
- `人物待确认`：标题、简介和已有字幕里暂时没有明确嘉宾；竞品模板下可理解为“对象待确认”。
...
- 高光建议：系统根据当前模板的高光词、观点密度和表达密度推荐片段。
- 导出序列视频：把剪辑序列里的片段拼成一个 MP4，并尽量烧录中文字幕。

## 典型工作流

1. 打开 `http://127.0.0.1:5173`。
2. 选择主题模板和日期区间，点击抓取按钮。
...
- `ai-interviews`：AI 采访、播客、对谈、炉边谈话。
- `tech-executive-interviews`：科技公司 CEO/CTO/创始人访谈、财报采访、峰会对话。
- `competitor-launches`：竞品发布会、产品 demo、hands-on 评测、roadmap、客户案例。

在页面顶部点击“模板设置”可以查看当前模板的 YouTube 查询词、B站查询词、相关词和高光词。内置模板建议先复制为自定义模板，再按自己的行业、公司、产品或人物名单调整。
